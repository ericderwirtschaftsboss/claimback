...
export function NavSidebar() {
  ...
  const navItems = [
    { href: '/dashboard', label: t('nav.myScans'), icon: LayoutDashboard },
    { href: '/scan', label: t('nav.scanContract'), icon: FileSearch },
    { href: '/compare', label: t('nav.compareVersions'), icon: GitCompare },
    { href: '/settings', label: t('nav.settings'), icon: Settings },
  ]

  return (
    <>
      <button className="fixed top-4 left-4 z-50 md:hidden p-2 rounded-md bg-card border" onClick={() => setMobileOpen(!mobileOpen)}>
        {mobileOpen ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}
      </button>
      <aside className={cn('fixed inset-y-0 left-0 z-40 w-64 bg-card border-r transform transition-transform md:translate-x-0', mobileOpen ? 'translate-x-0' : '-translate-x-full')}>
            ...
            {navItems.map((item) => (
              <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)}
                className={cn('flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium transition-colors',
                  pathname === item.href || pathname.startsWith(item.href + '/') ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:bg-accent hover:text-accent-foreground')}>
                <item.icon className="h-4 w-4" />{item.label}
              </Link>
            ))}
          </nav>
          <div className="px-3 py-4 border-t space-y-2">
            <div className="px-3 py-1"><LanguageToggle /></div>
